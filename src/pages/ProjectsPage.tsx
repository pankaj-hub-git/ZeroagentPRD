...
const catLabel: Record<string, string> = { apartment: 'APARTMENT', villa: 'VILLA', off_plan: 'OFF-PLAN' };
...
export function ProjectsPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await sb.from('xray_projects')
        .select('*')
  ...
  const selectorBg = isDark ? '#0c0d12' : colors.surface;

  return (
    <div style={{ color: colors.text }}>
      {/* PROJECT SELECTOR */}
      <div style={{ borderBottom: `1px solid ${colors.border}`, padding: '8px 12px', display: 'flex', gap: 0, overflowX: 'auto', background: selectorBg }}>
        {projects.map((p) => (
          <button key={p.id} onClick={() => setSel(p)} className="za-subtab" style={{
            padding: '8px 14px',
            color: sel?.id === p.id ? colors.gold : colors.textDim,
            borderBottomColor: sel?.id === p.id ? colors.gold : 'transparent',
            fontWeight: sel?.id === p.id ? 700 : 400,
            background: sel?.id === p.id ? colors.cardBg : 'transparent',
          }}>
            <span>{p.project_name}</span>
            <span style={{ marginLeft: 6, fontSize: 8, padding: '1px 5px', borderRadius: 2,
              background: (catColor[p.project_category] || colors.textDim) + '20',
              color: catColor[p.project_category] || colors.textDim,
      ...
      {sel && (
        <>
          {/* HERO */}
          <div style={{ padding: '44px 28px 32px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <h1 className="font-display" style={{ fontSize: 40, fontWeight: 300, margin: 0, letterSpacing: 1, lineHeight: 1.1, color: colors.text }}>
                {sel.project_name}
              </h1>
          ...
          <div style={{ maxWidth: 860, margin: '0 auto', padding: '32px 20px' }}>
            {detailLoading ? (
              ...
              <>
                    ...
                    <Section title="Villa Community Intelligence" accent colors={colors}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        <PCard colors={colors}>
                          ...
                          <div style={{ marginTop: 6 }}><Badge color={sel.villa_highway_noise_risk === 'low' ? colors.green : sel.villa_highway_noise_risk === 'moderate' ? colors.orange : colors.red} label={`NOISE: ${sel.villa_highway_noise_risk?.toUpperCase()}`} /></div>
                        </PCard>
                        <PCard colors={colors}>
                          <div className="za-data-label">NEAREST SCHOOL</div>
                          <div style={{ fontSize: 14, fontWeight: 400, color: colors.text, marginTop: 6 }}>{sel.villa_nearest_school}</div>
                          <div style={{ fontSize: 12, color: colors.gold, marginTop: 2 }}>{sel.villa_school_proximity_km}km away</div>
                        </PCard>
                        <PCard colors={colors}>
                          <div className="za-data-label">COMMUNITY MATURITY</div>
                          <div style={{ fontSize: 16, fontWeight: 400, color: colors.text, marginTop: 6, textTransform: 'capitalize' }}>{sel.villa_community_maturity?.replace(/_/g, ' ')}</div>
                ...
                {tab === 'offplan' && isOffplan && (
                  <div>
                    <Section title="Construction & Delivery Status" accent colors={colors}>
                      <PCard colors={colors}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                          <div style={{ padding: 12, background: colors.bg, borderRadius: 4, textAlign: 'center' }}>
                            <div className="za-data-label">DEVELOPER DELAY HISTORY</div>
                      ...
                      {sel.offplan_estimated_sc && (
                        <PCard colors={colors} style={{ marginTop: 12 }}>
                          <div className="za-data-label" style={{ color: colors.orange }}>ESTIMATED SERVICE CHARGE (RERA PLACEHOLDER — NOT MOLLAK CONFIRMED)</div>
                          <div className="za-data-value-lg" style={{ marginTop: 6 }}>~AED {Number(sel.offplan_estimated_sc).toFixed(1)}/sqft/year</div>
                          <div style={{ fontSize: 10, color: colors.textDim, marginTop: 4 }}>This is a RERA default estimate. Actual Mollak SC will only be published after handover. Expect ±30% variance.</div>
                        </PCard>
                      )}
                    </Section>
                  </div>
                ...
                {tab === 'evidence' && (
                  <div>
                    <Section title="DLD Transaction Evidence" subtitle="Real sales registered with Dubai Land Department. Not asking prices." accent colors={colors}>
                      {dldRecent.length === 0 ? (
                ...
                {tab === 'amenities' && (
                  <div>
                    <Section title="Amenity Delivery Assessment" subtitle="What was promised vs. what exists — verified from community sentiment data" accent colors={colors}>
                      <PCard colors={colors} style={{ marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          ...
                          <div style={{ fontSize: 12, color: colors.orange }}>{amenities.filter((a: R) => a.delivery_status === 'pending_verification').length} pending verification</div>
                        </div>
                      </PCard>
                      {amenities.length === 0 ? (
                        <PCard colors={colors}><div style={{ color: colors.textDim, textAlign: 'center', padding: 20 }}>Amenity data pending for this project</div></PCard>
                      ) : amenities.map((a, i) => (
                        ...
                        {infra.map((inf, i) => (
                          <PCard key={i} colors={colors} style={{ marginBottom: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                              ...
                              {inf.expected_completion && <span style={{ fontSize: 10, color: colors.orange, marginLeft: 8 }}>Est. {inf.expected_completion.slice(0, 4)}</span>}
                            </div>
                  ...
                  <div>
                    ...
                    <div className="za-signal-box za-signal-box--gold" style={{ textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: 11, color: colors.textSecondary, lineHeight: 1.6 }}>
                        Transactions: DLD via bronze.dld_transactions ({fmt(dldRecent.length)} recent).
                        Rentals: Ejari via bronze.rent_contracts_clean ({ejari.reduce((a: number, e: R) => a + e.contract_count, 0)} contracts).
                        Service charges: Mollak via bronze.mollak_service_charges ({sel.sc_source}).
                        Amenities: bronze.community_amenities_raw (community sentiment verified).
                      </div>
          ...
          <div className="za-footer">
            ...
            <span style={{ fontSize: 10, color: colors.textDim }}>Independent analysis. No commercial relationship with any developer or brokerage.</span>
          </div>
        </>
      )}
    </div>
